...
Example:
	persistenceCore testnet init-files --v 4 --output-dir ./.testnets --starting-ip-address 192.168.10.2
	`,
	...
	cmd.Flags().String(flagNodeDaemonHome, "persistenceCore", "Home directory of the node's daemon configuration")
...
Example:
	persistenceCore testnet --v 4 --output-dir ./.testnets
	`,
		RunE: func(cmd *cobra.Command, _ []string) error {
			args := startArgs{}
			args.outputDir, _ = cmd.Flags().GetString(flagOutputDir)
...
func collectGenFiles(
	...
	for i := 0; i < numValidators; i++ {
		...
		nodeAppState, err := genutil.GenAppStateFromConfig(clientCtx.Codec, clientCtx.TxConfig, nodeConfig, initCfg, appGenesis, genBalIterator, genutiltypes.DefaultMessageValidator, address.NewBech32Codec(constants.Bech32PrefixValAddr))
		if err != nil {
			return err
		}
