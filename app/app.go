...
func NewApplication(
	...
	govRouter.AddRoute(
		govtypes.RouterKey,
		govv1beta1types.ProposalHandler,
	).AddRoute(
...
func (app *Application) ExportAppStateAndValidators(forZeroHeight bool, jailWhiteList []string) (servertypes.ExportedApp, error) {
	...
	if forZeroHeight {
		...
		app.StakingKeeper.IterateValidators(context, func(_ int64, val stakingtypes.ValidatorI) (stop bool) {
			...
			err := app.DistributionKeeper.Hooks().AfterValidatorCreated(context, val.GetOperator())
			if err != nil {
				panic(err)
			}
			return false
		...
		for _, delegation := range delegations {
			...
			err = app.DistributionKeeper.Hooks().BeforeDelegationCreated(context, delegatorAddress, validatorAddress)
			if err != nil {
				panic(err)
			}
			err = app.DistributionKeeper.Hooks().AfterDelegationModified(context, delegatorAddress, validatorAddress)
			if err != nil {
				panic(err)
			}
		}
