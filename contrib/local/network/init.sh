...
rm -rf $CHAIN_DIR_1 &> /dev/null
rm -rf $CHAIN_DIR/$CHAINID_2 &> /dev/null

# Add directories for both chains, exit if an error occurs
if ! mkdir -p $CHAIN_DIR_1 2>/dev/null; then
...
$BINARY add-genesis-account $($BINARY keys show demowallet1 --keyring-backend test -a) 100000000000stake
...
$BINARY add-genesis-account $($BINARY keys show rly1 --keyring-backend test -a) 100000000000stake
...
$BINARY gentx val1 7000000000stake --chain-id $CHAINID_1 --keyring-backend test
...
jq -r ".app_state.wasm.params.code_upload_access.permission |= \"${wasm_permission}\"" $CHAIN_DIR_1/config/genesis.json > /tmp/genesis.json; mv /tmp/genesis.json $CHAIN_DIR_1/config/genesis.json
